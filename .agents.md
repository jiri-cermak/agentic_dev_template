...
**Skill isolation:** Rename `AGENTS.md` to `.agents.md` (dotfile). The runtime scans only `.hermes.md`, `AGENTS.md`, `CLAUDE.md`, and `.cursorrules` for auto-discovery. A dotfile is invisible to progressive subdirectory discovery, preventing the runtime from injecting it as a virtual skill. The relay context is the subagent's sole source of rules.
